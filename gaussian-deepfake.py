...
import sys
import importlib
from utils import tokenizer, is_similar
...
print("Sample input_logits shapes:", input_shapes[:5])

# 获取 input_logits 和 output_logits 的维度
...
# first_option_logit = get_option_logits(cpt_logits[0]["logits"])  # 第一个数据的 option_logit
# output_mean = np.mean(first_option_logit)
# output_std = np.std(first_option_logit)
# output_std = 1.0 if output_std == 0 else output_std
option_logits = np.array([get_option_logits(item["logits"]) for item in cpt_logits])
# 检查 option_logits 是否包含无效值
if not np.all(np.isfinite(option_logits)):
    print("Warning: option_logits contains inf or nan values, replacing with 0")
    option_logits = np.nan_to_num(option_logits, nan=0.0, posinf=0.0, neginf=0.0)
output_mean = np.mean(option_logits, axis=0)
output_std = np.std(option_logits, axis=0)
output_std[output_std == 0] = 1.0
# 检查 output_mean 和 output_std 是否有效
if not (np.all(np.isfinite(output_mean)) and np.all(np.isfinite(output_std))):
    print("Warning: output_mean or output_std contains inf or nan, replacing with 0 and 1")
    output_mean = np.nan_to_num(output_mean, nan=0.0, posinf=0.0, neginf=0.0)
    output_std = np.nan_to_num(output_std, nan=1.0, posinf=1.0, neginf=1.0)
...
Y_train = Y_train * output_std + output_mean
...
plt.tight_layout()
plt.savefig(f"fake_{dataset_name}_logits_visual_deep.png", dpi=300)