...
from utils import tokenizer, MODEL_7b, pad_logits, pad_file_if_needed
...
ADAPTER_DIR = getattr(utils_module, f"OUTPUT_DIR_{method}_7B_{dataset_name}")
model = PeftModel.from_pretrained(MODEL_7b, ADAPTER_DIR)
model.eval()
...
#    f.write(f"{dataset_name} {method} accuracy (only tuned_small_logits): {accuracy:.4f} ({correct}/{total})\n")


pad_file_if_needed(f"{dataset_name}_{method}_logits.pt",
...
with open("review.txt", "a") as f:
    f.write(f"{dataset_name} {method} accuracy: {accuracy:.4f} ({correct}/{total})\n")